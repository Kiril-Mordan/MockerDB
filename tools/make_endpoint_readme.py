...
def generate_markdown(data):
    ...
    for path, methods in data.get('paths', {}).items():
        for method, details in methods.items():
            # Create an anchor name based on path and method
            anchor_name = path.strip("/").replace("/", "_") + "_" + method
            toc.append(f"- [{method.upper()} {path}](#{anchor_name})")

            endpoint_str = f"\n### {details.get('summary', 'API Endpoint')}\n\n"
            ...
            for status_code, response in responses.items():
                ...
                response_example = json.dumps(response_content.get('example', {}), indent=2)
                endpoint_str += f"- **Response {status_code}**: {response_description}\n  ```json\n{response_example}\n  ```\n\n"

            endpoints.append(endpoint_str)

    # Combine Table of Contents and endpoint details
    return "\n".join(toc) + "\n" + "\n".join(endpoints)
